...
type PluginUiRouter struct {
	...
	EnableDevMode    bool      //Enable dev mode
	UIDevWebRoot     string    //The local web root used in dev mode
...
func (p *PluginUiRouter) SetDevWebRoot(devWebRoot string) {
	p.UIDevWebRoot = devWebRoot
	if devWebRoot != "" {
		p.EnableDevMode = true
	}
}

// readFile reads a file from either the dev web root or embedded FS
func (p *PluginUiRouter) readFile(relativePath string) ([]byte, error) {
	if p.EnableDevMode && p.UIDevWebRoot != "" {
		// Read from dev web root
		fullPath := p.UIDevWebRoot + "/" + relativePath
		fullPath = strings.ReplaceAll(fullPath, "//", "/")
		return os.ReadFile(fullPath)
	}
	// Read from embedded FS
	return fs.ReadFile(*p.TargetFs, relativePath)
}
...
func (p *PluginUiRouter) populateCSRFToken(r *http.Request, fsHandler http.Handler) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasSuffix(r.URL.Path, ".html") {
			...
			targetFileContent, err := p.readFile(targetFilePath)
			if err != nil {
			...
			indexFileContent, err := p.readFile(indexFilePath)
			if err == nil {
...
func (p *PluginUiRouter) Handler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if p.EnableDevMode && p.UIDevWebRoot != "" {
			// Serve from dev web root
			if p.EnableDebug {
				fmt.Println("Serving from dev web root:", p.UIDevWebRoot)
			}
			fileServer = http.FileServer(http.Dir(p.UIDevWebRoot))
		} else {
			// Serve from the embed.FS
		...
		p.populateCSRFToken(r, fileServer).ServeHTTP(w, r)
	})
}
